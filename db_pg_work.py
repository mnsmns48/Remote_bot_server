...
def get_price_on_server(table: Table) -> Sequence[Row[Any]]:
    engine = create_engine(f"postgresql://{hv.server_db_username_server}:{hv.server_db_password_server}"
                           f"@{hv.remote_bind_address_host}:{hv.remote_bind_address_port}/shippers")
    conn = engine.connect()
    sample = select(table.c.date, table.c.product, table.c.out_cost) \
        .order_by(desc(table.c.date)).order_by(table.c.out_cost).limit(250)
    res = conn.execute(sample).fetchall()
    conn.close()
    return res
